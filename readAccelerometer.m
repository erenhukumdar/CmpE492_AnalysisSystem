function readAccelerometer(folderPath,fileName,period)
%     [sensorMatrix char raw] = xlsread(fileName);
    clc;
   

    [accCalibreX,accCalibreY,accCalibreZ]=getCalibrationDataForAccelerometer([folderPath,'Calibration\AccelerometerSensorProbe.csv']);
    timeNormalize=0;
    close all;
...
%             zeroPos=find(periodDividedPoistions(:)==0);
%             periodDividedPoistions(zeroPos)=NaN;
...
%             varname=[folderPath,'ResultMatrix\',act,'\Accelerometer'];
%             save(varname,'resultMatrix');
            
            figure(figureCounter);
            plot(timeStamps,xValues,'g','LineWidth',3);
            hold on;
            plot(timeStamps,yValues,'c','LineWidth',3);
            hold on;
            ...
            ylabel('Axes Values');
            xlabel('Time (Seconds)');
            title(['Accelerometer Raw Values vs Time',uniqueMacIDs(i),'-',uniqueActivities(j)]);
            ...
            xlabel('Time (Seconds)');
            title(['Accelerometer Rate of Change vs Time for',uniqueMacIDs(i),'-',uniqueActivities(j)]);
            subplot(1,3,2);
            ...
            disp(' ');
            disp('1) Variance Values of Accelerometer Sensor');
